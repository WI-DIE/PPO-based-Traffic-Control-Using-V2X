...
from config import ENTORPYCOEFF
...
class PPOAgent:
    def __init__(self, state_dim, action_dim, hidden_dim, gamma=0.99, lr=3e-4, clip_eps=0.2, epochs=10):
        ...
        #self.scheduler_policy = CosineAnnealingLR(
        #    self.optimizer_policy, T_max=300, eta_min=1e-6)
        # 하이퍼파라미터 설정
        self.gamma = gamma
    ...
    def update(self, next_state, done):
        ...
        for i, param_group in enumerate(self.optimizer_policy.param_groups):
            print(
                f"[POLICY OPTIMIZER] Learning Rate (group {i}): {param_group['lr']:.6f}")

        for i, param_group in enumerate(self.optimizer_value.param_groups):
            print(
                f"[VALUE OPTIMIZER] Learning Rate (group {i}): {param_group['lr']:.6f}")
        # print("Current LR:", self.scheduler_policy.get_last_lr())
        ...
        for _ in range(self.epochs):
            ...
            entropy_coeff = ENTORPYCOEFF  # 탐험 비중 조절 하이퍼파라미터 (값이 크면 탐험 증가)
            ...
            self.writer.add_scalar(
                "LR/Policy", self.optimizer_policy.param_groups[0]['lr'], self.total_steps)
            self.writer.add_scalar(
                "LR/Value", self.optimizer_value.param_groups[0]['lr'], self.total_steps)
